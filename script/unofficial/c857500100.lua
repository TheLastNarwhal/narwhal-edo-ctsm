--Chaos Goddess, the Sovereign of the Chaotic World
...
function s.initial_effect(c)
	...
	--Special Summon 1 level 4 or lower DARK monster that is banished or in your GY
	...
	c:RegisterEffect(e1)
	--Special Summon 1 Level 5 or higher DARK monster from your GY
...
function s.spfilter(c,e,tp)
	return c:IsFaceup() and c:IsLevelBelow(4) and c:IsAttribute(ATTRIBUTE_DARK) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
	--Activation legality
...
function s.costfilter(c)
	return c:IsAttribute(ATTRIBUTE_LIGHT) and c:IsType(TYPE_MONSTER) and c:IsAbleToGraveAsCost()
end
	--Activation legality
...
function s.tdtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	local g=Duel.SelectTarget(tp,aux.NecroValleyFilter(s.tdfilter),tp,LOCATION_GRAVE+LOCATION_REMOVED,0,1,1,e:GetHandler())
