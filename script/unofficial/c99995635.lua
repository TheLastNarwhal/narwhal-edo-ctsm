...
s.listed_names={99995636}
--Special Summon from hand by sending 1 Ritual monster from Deck to GY
function s.spfilter(c,tp)
	return (c:IsType(TYPE_RITUAL) and c:IsType(TYPE_MONSTER)) and c:IsAbleToGraveAsCost() and (Duel.GetLocationCount(tp,LOCATION_MZONE)>0 or (c:IsLocation(LOCATION_MZONE) and c:GetSequence()<5))
end
function s.spcon(e,c)
	if c==nil then return true end
	local tp=c:GetControler()
	local rg=Duel.GetMatchingGroup(s.spfilter,tp,LOCATION_DECK,0,nil,tp)
...
function s.filter(c)
	return c:IsCode(99995636) and c:IsAbleToHand()
end
...
function s.mtcon(e,tp,eg,ep,ev,re,r,rp)
	return r==REASON_RITUAL and e:GetHandler():IsPreviousLocation(LOCATION_MZONE)
		and eg:IsExists(Card.IsSetCard,1,nil,0x195)
end
