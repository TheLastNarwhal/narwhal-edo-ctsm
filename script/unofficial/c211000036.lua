--Danger Dungeon! Black Pudding!?
--Scripted by Narwhal
Duel.LoadScript("cstm_card_specific_functions.lua")
local s,id=GetID()
function s.initial_effect(c)
    --Destroy 1 card your opponent controls, Special Summon this card
    ...
    c:RegisterEffect(e1)
    --Add 1 "Danger Dungeon!" monster from Deck to hand
...
function s.sphtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
    ...
    if chk==0 then return Duel.IsExistingTarget(nil,tp,0,LOCATION_ONFIELD,1,nil,tp) and e:GetHandler():IsCanBeSpecialSummoned(e,0,tp,false,false) and Duel.GetLocationCount(tp,LOCATION_MZONE)>0 end
...
function s.thcon(e,tp,eg,ep,ev,re,r,rp)
    return Duel.GetFieldGroupCount(tp,LOCATION_MZONE,0)<=1 or not Duel.IsExistingMatchingCard(aux.NOT(aux.FaceupFilter(Card.IsSetCard,SET_DANGER_DUNGEON)),tp,LOCATION_MZONE,0,1,nil)
end
function s.thfilter(c)
    return c:IsSetCard(SET_DANGER_DUNGEON) and c:IsMonster() and not c:IsCode(id) and c:IsAbleToHand()
end
